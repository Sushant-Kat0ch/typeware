...
@Component({
    ...
    template: `<div class="col-span-12 lg:col-span-6 xl:col-span-3">
            <div class="card mb-0">
                <div class="flex justify-between mb-4" [innerHTML]="currentPara.join(',')">
                    <!-- <p>{{currentPara}}</p> -->
        </div>`
})
export class ParaType implements OnInit {
    ...
    public currentPara: string[] = this.paragraphs[0].split('').map(key => `<span class="default">${key}</span>`)
    ...
    onKeyDown(event: KeyboardEvent) {
        ...
        if (this.key === this.currentKey[this.typed()].toLocaleLowerCase()) { // Compare with current key
            ...
            this.currentPara[this.typed()] = `<span class="default">${this.currentKey[this.typed()]}</span>`  
